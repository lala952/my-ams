    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain :disabled="multiple" @click="handleBatchApprove"
                   v-hasPermi="['asset:disposal:approve']">批量审批
        </el-button>
      </el-col>
      <el-col :span="1.5">
      ...
      <el-table-column label="操作" align="center" width="170" fixed="right">
        <template #default="scope">
          <el-button link type="primary" size="small" @click="handleApprove(scope.row)"
                     v-if="scope.row.businessStatus === 'pending'" v-hasPermi="['asset:disposal:approve']">审批
          </el-button>
...
<script setup name="DisposalApprove">
import {computed, getCurrentInstance, onMounted, reactive, ref, toRefs} from 'vue'
import {useRouter} from 'vue-router'
import {ElMessage, ElMessageBox} from 'element-plus'
import {batchApprove, countByStatus, listDisposal} from '@/api/asset/disposal'
...
const statusOptions = computed(() => {
  return disposal_business_status.value
...
function getList() {
  ...
  listDisposal(proxy.addDateRange(queryParams.value, dateRange.value))
      .then((res) => {
...
function handleApprove(row) {
  router.push({
    path: "/asset/approve/center/disposal/approve/detail",
    query: {id: row.id}
  })
}
