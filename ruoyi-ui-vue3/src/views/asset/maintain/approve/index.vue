  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
      <el-form-item label="维修单号" prop="maintainCode">
        <el-input v-model="queryParams.maintainCode" placeholder="请输入维修单号" clearable
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain :disabled="multiple" @click="handleBatchApprove"
                   v-hasPermi="['asset:maintain:approve']">批量审批
        </el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table v-loading="loading" :data="maintainList" @selection-change="handleSelectionChange" border>
      ...
      <el-table-column label="维修单号" prop="maintainCode" show-overflow-tooltip align="center"
      >
      ...
      <el-table-column align="right" label="预计费用" prop="estimatedCost">
        <template #default="scope">
          <span>{{ formatMoney(scope.row.estimatedCost) }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="170" fixed="right">
        <template #default="scope">
          <el-button link type="primary" size="small" @click="handleApprove(scope.row)"
                     v-if="scope.row.businessStatus === 'pending'" v-hasPermi="['asset:maintain:approve']">审批
          </el-button>
...
<script setup name="MaintainApprove">
import {computed, getCurrentInstance, onMounted, reactive, ref, toRefs} from 'vue'
import {useRouter} from 'vue-router'
import {ElMessage, ElMessageBox} from 'element-plus'
import {batchApprove, countByStatus, listMaintain} from '@/api/asset/maintain'
...
const statusOptions = computed(() => {
  return maintain_business_status.value
...
function getList() {
  ...
  listMaintain(proxy.addDateRange(queryParams.value, dateRange.value))
      .then((res) => {
...
function handleApprove(row) {
  router.push({
    path: "/asset/approve/center/maintain/approve/detail",
    query: {id: row.id}
  })
}
...
function handleExport() {
  proxy.download(
      "asset/maintain/export",
      {...queryParams.value},
      `资产维修审批_${new Date().getTime()}.xlsx`
  )
}
