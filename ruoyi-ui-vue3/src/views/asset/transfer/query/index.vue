<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
      ...
      <el-form-item label="调出部门" prop="outDeptName">
        <el-input v-model="queryParams.outDeptName" placeholder="请输入调出部门" clearable @keyup.enter="handleQuery"
                  style="width: 180px"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="info" plain @click="handleExport" v-hasPermi="['asset:transfer:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" width="150" fixed="right">
        <template #default="scope">
          ...
          <el-button link type="primary" size="small" @click="handleExportPdf(scope.row)"
                     v-hasPermi="['asset:transfer:export']">打印
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="TransferQuery">
import {computed, getCurrentInstance, onMounted, reactive, ref, toRefs} from 'vue'
import {useRouter} from 'vue-router'
import {ElMessage} from 'element-plus'
import {countByStatus, listTransfer} from '@/api/asset/transfer'
...
const statusOptions = computed(() => {
  return transfer_business_status.value.map(item => ({
    label: item.label,
    value: item.value
  }))
})
...
function getList() {
  ...
  listTransfer(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
...
function handleView(row) {
  router.push({
    path: '/asset/query/center/transfer/query/detail',
    query: {id: row.id}
  })
}

function handleExport() {
  proxy.download('asset/transfer/export', {
    ...queryParams.value
  }, `资产调拨申请_${new Date().getTime()}.xlsx`)
}
