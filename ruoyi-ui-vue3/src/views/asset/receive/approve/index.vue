    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain :disabled="multiple" @click="handleBatchApprove"
                   v-hasPermi="['asset:receive:approve']">批量审批
        </el-button>
      </el-col>
      <el-col :span="1.5">
      ...
      <el-table-column label="操作" align="center" width="170" fixed="right">
        <template #default="scope">
          <el-button link type="primary" size="small" @click="handleApprove(scope.row)"
                     v-if="scope.row.businessStatus === 'pending'" v-hasPermi="['asset:receive:approve']">审批
          </el-button>
...
<script setup name="ReceiveApprove">
import {computed, getCurrentInstance, onMounted, reactive, ref, toRefs} from 'vue'
import {useRouter} from 'vue-router'
import {ElMessage, ElMessageBox} from 'element-plus'
import {batchApprove, countByStatus, listReceive} from '@/api/asset/receive'
...
const statusOptions = computed(() => {
  return receive_business_status.value
...
function getList() {
  ...
  listReceive(proxy.addDateRange(queryParams.value, dateRange.value))
      .then((res) => {
...
function handleApprove(row) {
  router.push({
    path: "/asset/approve/center/receive/approve/detail",
    query: {id: row.id}
  })
}
...
function handleExport() {
  proxy.download(
      "asset/receive/export",
      {...queryParams.value},
      `资产领用审批_${new Date().getTime()}.xlsx`
  )
}
