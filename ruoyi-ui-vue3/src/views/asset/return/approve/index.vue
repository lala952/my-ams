<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
      <el-form-item label="交回单号" prop="returnCode">
        <el-input v-model="queryParams.returnCode" placeholder="请输入交回单号" clearable @keyup.enter="handleQuery"
                  style="width: 180px"/>
      </el-form-item>
      <el-form-item label="交回人" prop="returnPersonName">
        <el-input v-model="queryParams.returnPersonName" placeholder="请输入交回人" clearable @keyup.enter="handleQuery"
                  style="width: 180px"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain :disabled="multiple" @click="handleBatchApprove"
                   v-hasPermi="['asset:return:approve']">批量审批
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="info" plain @click="handleExport" v-hasPermi="['asset:return:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="returnList" @selection-change="handleSelectionChange" border>
      ...
      <el-table-column align="center" label="交回单号" prop="returnCode" show-overflow-tooltip>
        <template #default="scope">
          <a class="link-type" @click="handleView(scope.row)">{{ scope.row.returnCode }}</a>
        </template>
      </el-table-column>
      <el-table-column label="交回人" prop="returnPersonName" show-overflow-tooltip/>
      <el-table-column label="交回部门" prop="returnDeptName" show-overflow-tooltip/>
      <el-table-column align="center" label="交回日期" prop="returnDate">
        <template #default="scope">
      ...
      <el-table-column align="center" label="审批状态" prop="businessStatus">
        <template #default="scope">
          <ApprovalStatusBubble
              ...
              :business-status-dict="return_business_status"
          />
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="170" fixed="right">
        <template #default="scope">
          <el-button link type="primary" size="small" @click="handleApprove(scope.row)"
                     v-if="scope.row.businessStatus === 'pending'" v-hasPermi="['asset:return:approve']">审批
          </el-button>
...
<script setup name="ReturnApprove">
import {computed, getCurrentInstance, onMounted, reactive, ref, toRefs} from 'vue'
import {useRouter} from 'vue-router'
import {ElMessage, ElMessageBox} from 'element-plus'
import {batchApprove, countByStatus, listReturn} from '@/api/asset/return'
import ApprovalStatusBubble from '@/components/Asset/BusinessStatusPopover/index.vue'

const {proxy} = getCurrentInstance()
const router = useRouter()

const {return_business_status} = proxy.useDict('return_business_status')

// 根据字典动态生成Tab选项(排除'草稿'状态)
const statusOptions = computed(() => {
  return return_business_status.value
...
function getList() {
  ...
  listReturn(proxy.addDateRange(queryParams.value, dateRange.value))
      .then((res) => {
...
function handleApprove(row) {
  router.push({
    path: "/asset/approve/center/return/approve/detail",
    query: {id: row.id}
  })
}
