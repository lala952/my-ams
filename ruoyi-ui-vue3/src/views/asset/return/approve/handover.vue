<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
      <el-form-item label="交接单号" prop="handoverCode">
        <el-input v-model="queryParams.handoverCode" placeholder="请输入交接单号" clearable @keyup.enter="handleQuery"
                  style="width: 180px"/>
      </el-form-item>
      <el-form-item label="交出人" prop="outUserName">
        <el-input v-model="queryParams.outUserName" placeholder="请输入交出人" clearable @keyup.enter="handleQuery"
                  style="width: 180px"/>
      </el-form-item>
      <el-form-item>
    ...
    <!-- 数据表格 -->
    <el-table v-loading="loading" :data="handoverList" border @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="50" align="center" fixed="left"/>
      <el-table-column label="序号" type="index" width="60" align="center" fixed="left"/>
      <el-table-column align="center" label="交接单号" prop="handoverCode" show-overflow-tooltip>
        <template #default="scope">
          <a class="link-type" @click="handleView(scope.row)">{{ scope.row.handoverCode || '-' }}</a>
        </template>
      </el-table-column>
      <el-table-column label="交出人" prop="outUserName" show-overflow-tooltip/>
      <el-table-column label="接收人" prop="inUserName" show-overflow-tooltip/>
      <el-table-column label="交接部门" prop="deptName" show-overflow-tooltip/>
      <el-table-column align="center" label="资产数量" prop="assetCount" width="100"/>
      <el-table-column align="center" label="交接日期" prop="handoverDate">
        <template #default="scope">
    ...
    <!-- 详情抽屉 -->
    <el-drawer v-model="drawerVisible" title="交接详情" size="50%" direction="rtl">
      <div class="detail-content">
        <el-descriptions :column="2" border>
          <el-descriptions-item label="交接单号">{{ viewData.handoverCode || '-' }}</el-descriptions-item>
          <el-descriptions-item label="交出人">{{ viewData.outUserName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="接收人">{{ viewData.inUserName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="交接部门">{{ viewData.deptName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="资产数量">{{ viewData.assetCount || '-' }}</el-descriptions-item>
          <el-descriptions-item label="交接日期">{{
              parseTime(viewData.handoverDate, '{y}-{m}-{d}') || '-'
            }}
    ...
    <!-- 审批对话框 -->
    <el-dialog title="交接审批" v-model="approveOpen" width="600px" append-to-body>
...
<script setup name="HandoverApprove">
import {getCurrentInstance, reactive, ref, toRefs} from 'vue'
import {ElMessage} from 'element-plus'
import {batchApprove, getHandover, listHandover} from '@/api/asset/handover'
...
function getList() {
  loading.value = true
  listHandover(queryParams.value).then(res => {
