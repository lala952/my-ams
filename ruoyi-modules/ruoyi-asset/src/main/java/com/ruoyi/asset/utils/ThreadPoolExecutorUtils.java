 ...
 *
 * 八、面试的一些知识点总结
 * 1、线程池：管理一系列线程的资源池。当有任务要处理时，直接从线程池中获取线程来处理，
 *   处理完之后线程并不会立即被销毁，而是等待下一个任务。
 * 2、为啥使用线程池？
 * （1）降低资源消耗。线程池里的线程是可以重复利用的。一旦线程完成了某个任务，它不会立即销毁，而是回到池子里等待下一个任务。避免了频繁创建和销毁线程带来的开销。
 * （2）提高响应速度。线程池里通常维护一定数量的核心线程，任务来了之后，可以直接提交给这些已经存在的、空闲的线程去执行，省去了创建线程的时间，任务能够更快地得到处理。
 * （3）提交线程的可管理型。线程池允许我们统一管理池中的线程。我们可以配置线程池的大小（corePoolSize、maximumPoolSize）、任务队列的类型和大小、拒绝策略等。这样就能控制并发线程的总量，防止资源耗尽，保证系统的稳定性。同时，线程池通常也提供了监控接口，方便我们了解线程池的运行状态（如有多少活跃线程、多少任务在排队等），便于调优。
 */
